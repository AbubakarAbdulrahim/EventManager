...
function BookingDialog({ open, handleClose, service, onConfirm, addBooking }) {
  ...
  const calculatedPrice = calculatePrice(service?.basePrice, duration);
  console.log(`Duration changed to ${duration}, calculated price: ${calculatedPrice}`);
  
  setBookingData(prev => ({ 
    ...prev, 
    duration, 
    price: calculatedPrice  // Store the calculated price in the state
  }));
...
const checkTimeSlotAvailability = async (time, duration) => {
  ...
  try {
    ...
    // Use the price from bookingData state instead of recalculating
    const paymentAmount = bookingData.price;
    console.log("Payment amount from state:", paymentAmount);
    
    // Fall back to calculation if price is somehow not in the state
    if (!paymentAmount) {
      console.warn("Price not found in bookingData, recalculating");
    }
    
    window.FlutterwaveCheckout({
      public_key: import.meta.env.VITE_PUBLIC_KEY,
      tx_ref: Date.now(),
      amount: paymentAmount || calculatePrice(service?.basePrice, bookingData.duration),
      currency: "NGN",
